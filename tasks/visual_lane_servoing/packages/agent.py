...
class LaneServoingAgent:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.p_gain = cfg.get('p_gain', 0.08)
        self.d_gain = cfg.get('d_gain', 0.3)
        self.max_steer = cfg.get('max_steer', 0.35)
        self.base_speed = cfg.get('base_speed', 0.16)
        self.curve_speed = cfg.get('curve_speed', 0.2)
        self.curve_threshold = cfg.get('curve_threshold', 200)
        self.steering_threshold = cfg.get('steering_threshold', 0.2)
        self.curve_boost = cfg.get('curve_boost', 1.5)
...
# steering_threshold: 0.2
    ...
    def _calculate_steering(self, error: float) -> float:
        error_diff = (error - self._prev_error) * 0.5 + 0.5 * getattr(self, "_prev_diff", 0.0)
    ...
    def compute_commands(self, image: np.ndarray) -> Tuple[float, float]:
        ...
        self._filtered_error = 0.85 * self._filtered_error + 0.15 * raw_error
